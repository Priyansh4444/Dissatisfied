...
const getStylesheet = (): HTMLLinkElement | null => getElementById<HTMLLinkElement>(STYLESHEET_ID);
...
const getInjectionTarget = (): HTMLElement | null => {
  ...
  const tabList = queryElement<HTMLElement>("[data-testid='ScrollSnap-SwipeableList']");
  ...
  if (stickyHeader) {
    ...
    const flexRow = stickyHeader.querySelector<HTMLElement>(".css-175oi2r.r-1awozwy.r-18u37iz");
    if (flexRow) {
      return flexRow;
    }
  }
...
const getStorageValue = async <T>(key: string, defaultValue: T): Promise<Result<T>> => {
...
const getAllTwitterSettings = async (): Promise<TwitterSettings> => {
  ...
  const width = getNumberSetting(raw[STORAGE_KEYS.TWITTER_WIDTH], DEFAULTS.TWITTER_WIDTH);
...
const applyTwitterSettingsToDocument = (settings: TwitterSettings): void => {
  ...
  if (!settings.useDefaultFont) {
    ...
    root.style.setProperty("--twitter-icon-scale", String(1 + (settings.fontSize / 15 - 1) * 0.5));
  } else {
  ...
  root.classList.toggle("dissatisfied-twitter-hide-header", Boolean(settings.controls.hideHeader));
  root.classList.toggle(
    "dissatisfied-twitter-center-timeline",
...
const isActive = (): boolean => document.documentElement.classList.contains(ACTIVE_CLASS);
