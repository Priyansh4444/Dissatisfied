...
function bindUi() {
  ...
  for (const input of dom.youtubeModeInputs) {
    input.addEventListener("change", (e) =>
      saveSetting(STORAGE_KEYS.YOUTUBE_PERSISTENCE_MODE, e.target.value, "Saved"),
    );
  }

  for (const input of dom.twitterModeInputs) {
    input.addEventListener("change", (e) =>
      saveSetting(STORAGE_KEYS.TWITTER_PERSISTENCE_MODE, e.target.value, "Saved"),
    );
  }
...
async function loadSiteSettings(siteName) {
  try {
    if (siteName === "youtube") {
      const result = await chrome.storage.local.get([STORAGE_KEYS.YOUTUBE_PERSISTENCE_MODE]);
      const mode =
    ...
    if (siteName === "twitter") {
      ...
      const width = sanitizeNumber(result[STORAGE_KEYS.TWITTER_WIDTH], DEFAULTS.TWITTER_WIDTH);
      ...
      const controls = sanitizeTwitterControls(result[STORAGE_KEYS.TWITTER_FOCUS_CONTROLS]);
