...
function cacheDom() {
  ...
  dom.twitterControlInputs = Array.from(document.querySelectorAll("input[data-control-key]"));
...
function bindSettingsEvents() {
  ...
  if (dom.twitterFontSizeSlider) {
    dom.twitterFontSizeSlider.addEventListener("input", handleTwitterFontSizeInput);
    dom.twitterFontSizeSlider.addEventListener("change", handleTwitterFontSizeChange);
  }

  if (dom.twitterUseDefaultFont) {
    dom.twitterUseDefaultFont.addEventListener("change", handleTwitterUseDefaultFontChange);
  }
}
...
async function setInitialActiveSiteForCurrentTab() {
  try {
    ...
    if (chrome.storage?.local?.get) {
      ...
      const isValidSite = site === "youtube" || site === "twitter" || site === "excalidraw";
      const isFresh = typeof at === "number" && Date.now() - at < 15_000;

      if (isValidSite && isFresh) {
        setActiveSite(site);
        if (chrome.storage?.local?.remove) {
          await chrome.storage.local.remove([UI_KEYS.LAST_OPEN_SITE, UI_KEYS.LAST_OPEN_SITE_AT]);
        }
      }
    }
  } catch (error) {
...
async function loadSettings() {
  try {
    ...
    const youtubePersistenceMode =
      result[STORAGE_KEYS.YOUTUBE_PERSISTENCE_MODE] || DEFAULTS.YOUTUBE_PERSISTENCE_MODE;
    const twitterPersistenceMode =
      result[STORAGE_KEYS.TWITTER_PERSISTENCE_MODE] || DEFAULTS.TWITTER_PERSISTENCE_MODE;
    const twitterWidth = sanitizeNumber(result[STORAGE_KEYS.TWITTER_WIDTH], DEFAULTS.TWITTER_WIDTH);
    const twitterFontSize = sanitizeNumber(
      result[STORAGE_KEYS.TWITTER_FONT_SIZE],
      DEFAULTS.TWITTER_FONT_SIZE,
    );
    const twitterUseDefaultFont = result[STORAGE_KEYS.TWITTER_USE_DEFAULT_FONT] !== false;
    const twitterControls = sanitizeTwitterControls(result[STORAGE_KEYS.TWITTER_FOCUS_CONTROLS]);
...
function applyTwitterFontSize(fontSize) {
  ...
  document.documentElement.style.setProperty("--twitter-font-size", `${safeFontSize}px`);
}
...
async function handleYoutubePersistenceModeChange(event) {
  ...
  await saveSetting(STORAGE_KEYS.YOUTUBE_PERSISTENCE_MODE, mode, "YouTube preference saved");
}

async function handleTwitterPersistenceModeChange(event) {
  const mode = event.target.value;
  await saveSetting(STORAGE_KEYS.TWITTER_PERSISTENCE_MODE, mode, "Twitter/X preference saved");
}

async function handleTwitterControlChange() {
  const controls = getTwitterControlsFromForm();
  await saveSetting(STORAGE_KEYS.TWITTER_FOCUS_CONTROLS, controls, "Twitter/X controls saved");
}
...
function handleTwitterFontSizeInput(event) {
  const fontSize = sanitizeNumber(event.target.value, DEFAULTS.TWITTER_FONT_SIZE);
...
async function handleTwitterFontSizeChange(event) {
  ...
  await saveSetting(STORAGE_KEYS.TWITTER_FONT_SIZE, fontSize, "Twitter/X font size saved");
}
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  ...
  if (changes[STORAGE_KEYS.TWITTER_WIDTH]) {
    applyTwitterWidth(
      sanitizeNumber(changes[STORAGE_KEYS.TWITTER_WIDTH].newValue, DEFAULTS.TWITTER_WIDTH),
    );
  }

  if (changes[STORAGE_KEYS.TWITTER_FONT_SIZE]) {
    applyTwitterFontSize(
      sanitizeNumber(changes[STORAGE_KEYS.TWITTER_FONT_SIZE].newValue, DEFAULTS.TWITTER_FONT_SIZE),
    );
  }

  if (changes[STORAGE_KEYS.TWITTER_FOCUS_CONTROLS]) {
    applyTwitterControls(
      sanitizeTwitterControls(changes[STORAGE_KEYS.TWITTER_FOCUS_CONTROLS].newValue),
    );
  }

  if (changes[STORAGE_KEYS.TWITTER_USE_DEFAULT_FONT]) {
    applyTwitterUseDefaultFont(changes[STORAGE_KEYS.TWITTER_USE_DEFAULT_FONT].newValue !== false);
  }
});
