...
import { DEFAULTS, STORAGE_KEYS, isTwitterUrl, isYouTubeUrl } from "./shared/extension-settings.ts";
...
browser.runtime.onInstalled.addListener(async ({ reason }: { reason: string }) => {
  if (reason === "install") {
    ...
    await browser.storage.local.set({
      [STORAGE_KEYS.YOUTUBE_PERSISTENCE_MODE]: DEFAULTS.YOUTUBE_PERSISTENCE_MODE,
      [STORAGE_KEYS.TWITTER_PERSISTENCE_MODE]: DEFAULTS.TWITTER_PERSISTENCE_MODE,
      [STORAGE_KEYS.TWITTER_WIDTH]: DEFAULTS.TWITTER_WIDTH,
      [STORAGE_KEYS.TWITTER_FONT_SIZE]: DEFAULTS.TWITTER_FONT_SIZE,
      [STORAGE_KEYS.TWITTER_USE_DEFAULT_FONT]: DEFAULTS.TWITTER_USE_DEFAULT_FONT,
...
async function applyTwitterStyles(tabId: number): Promise<void> {
  try {
    const didContentScriptHandleIt = await sendTwitterContentAction(tabId, "apply-twitter-styles");
    ...
    if (!didContentScriptHandleIt) {
      ...
      const storedFontSize = await getStoredState(STORAGE_KEYS.TWITTER_FONT_SIZE);
      ...
      const storedControls = await getStoredState(STORAGE_KEYS.TWITTER_FOCUS_CONTROLS);
...
async function removeTwitterStyles(tabId: number): Promise<void> {
  try {
    const didContentScriptHandleIt = await sendTwitterContentAction(tabId, "remove-twitter-styles");
